...
OAuth 테스터 초대에 장애가 있고 이미 정상 발급된 Threads **사용자 액세스 토큰**을 보유한 경우에만 `ALLOW_MANUAL_THREADS_TOKEN=true`를 설정할 수 있습니다. 계정 카드의 `사용자 토큰 연결`에서 계정별 토큰을 입력하면 서버가 공식 `/me` API로 사용자 ID와 사용자명을 확인한 뒤 동일한 AES-256-GCM 저장소에 암호화합니다. `TH|`로 시작하는 앱 액세스 토큰은 oEmbed 같은 앱 단위 API용이므로 게시 계정 토큰으로 받지 않습니다. 수동 토큰은 만료 전에 같은 화면에서 교체해야 합니다.
